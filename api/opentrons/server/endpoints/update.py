...
ignored_updates_file = os.path.join(PATH, 'ignore.json')
...
def _set_ignored_version(version):
    ...
    with open(ignored_updates_file, 'w') as data_file:
...
def _get_ignored_version():
    ...
    """
    if os.path.exists(ignored_updates_file):
        with open(ignored_updates_file) as data_file:
