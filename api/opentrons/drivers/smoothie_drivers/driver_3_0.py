...
class SmoothieDriver_3_0_0:
    ...
    def horizontal_arc(self, target, arc_center, clockwise=True):
        '''
        ...
        # default to using the current position for both target and arc-center
        arc_target = {}
        for ax in 'XY':
            arc_target[ax] = round(
                ...
                GCODE_ROUNDING_PRECISION)
            arc_center[ax] = round(
                # convert arc-center coords to relative from current position
                arc_center.get(ax, self.position[ax]) - self.position[ax],
                GCODE_ROUNDING_PRECISION)
        ...
        # this gcode only actuates the XY axes, so explicitly dwell others
